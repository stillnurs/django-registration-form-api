...
class LoginSerializer(serializers.Serializer):
    ...
    class Meta:
        model = User
        fields = ['email', 'username', 'first_name', 'last_name',  'token']
...
class ProfileSerializer(serializers.Serializer):
    prefix = serializers.CharField(max_length=10, min_length=3)
    token = serializers.CharField(max_length=256, min_length=3)
    ...
    def validate(self, data):
        ...
        if self.prefix is None:
            raise serializers.ValidationError(
                'A prefix is required to retrieve user data.'
            )
